      ...
      <div className="container hero-inner">
        ...
        >
          <motion.div variants={fadeUp} transition={{ duration: 0.7 }} className="hero-vline">
            ...
            <div>
              <h1>O primeiro clube de <em>vela</em> do Brasil.</h1>
              <p className="hero-lead">Tradição, lazer e experiências inesquecíveis para toda a família.</p>
  ...
  return (
    <div className="wa-float" ref={ref}>
      ...
      <div className="wa-fab-group">
        ...
        {!open && <span className="wa-label">Chat</span>}
      </div>
    </div>
  )
}
    ...
    <footer className="footer" id="contato">
      <div className="container footer-inner">
        <div className="footer-brand">
          ...
          <p style={{ fontSize: 14, fontStyle: 'italic', lineHeight: 1.7, marginBottom: 12 }}>
...
function InstalacoeHome() {
  ...
  return (
    <section className="section section-alt" id="instalacoes-home">
      <div className="container">
        <Reveal>
          <div className="section-head" style={{ marginBottom: 28 }}>
            <div>
              <div className="eyebrow eyebrow-gold">Instalações</div>
              <h2>Nossa <em style={{ fontStyle: 'italic', color: 'var(--brass)' }}>estrutura</em></h2>
            </div>
            <a className="section-link" href="#instalacoes">Ver todas <ArrowRight size={15} /></a>
          </div>
        </Reveal>
        <Reveal>
          <div className="inst-photo-grid">
            {instalacoes.slice(0, 5).map((inst) => {
              const fotos = inst.fotos ?? []
              const firstFoto = fotos.length > 0 ? urlForImage(fotos[0]) : ''
              return (
                <div key={inst._id} className="inst-photo-card" onClick={() => { window.location.hash = 'instalacoes' }}>
                  <div className="inst-photo-area">
                    {firstFoto
                      ? <img src={firstFoto} alt={inst.titulo} draggable={false} />
                      : <div style={{ width: '100%', height: '100%', background: 'linear-gradient(160deg,#1a3a5c,#0a2742)' }} />
                    }
                    <div className="inst-photo-overlay">
                      <div className="inst-photo-name">{inst.titulo}</div>
                      {inst.ctaLabel
                        ? <div className="inst-photo-cta">{inst.ctaLabel} »</div>
                        : <div className="inst-photo-cta">Conhecer »</div>
                      }
                    </div>
                  </div>
                </div>
